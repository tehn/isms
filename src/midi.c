// see https://fundamental-code.com/midi for byte decoding
...
void handle_event() {
  ...
  if (midi.sev->type == SND_SEQ_EVENT_PORT_START) {
    ...
    fprintf(stderr, ">> MIDI: new midi port %i:%i\n", midi.sev->data.addr.client,
        midi.sev->data.addr.port);

    error = snd_seq_get_any_port_info(midi.seq, midi.sev->data.addr.client,
        midi.sev->data.addr.port, port_info);

    if (error < 0) {
      fprintf(stderr, ">> MIDI: failed to get port information\n");
    }
    ...
    if ((port_caps & SND_SEQ_PORT_CAP_READ) && (port_caps & SND_SEQ_PORT_CAP_SUBS_READ)) {
      error = snd_seq_connect_from(midi.seq, midi.input_port,
          midi.sev->data.addr.client, midi.sev->data.addr.port);

      if (error < 0) {
        fprintf(stderr, ">> MIDI: failed to connect to port\n");
      }
    }
  ...
  // TODO: don't ignore sysex
  if (snd_seq_ev_is_subscribe_type(midi.sev) || snd_seq_ev_is_variable_type(midi.sev)) {
    fprintf(stderr, ">> MIDI: ignoring midi event of type %d\n", midi.sev->type);
    return;
  }

  ev = event_data_new(EVENT_MIDI);
  nbytes = snd_midi_event_decode(midi.parser, ev->midi.data, 3, midi.sev);
...
void init_midi() {
  ...
  snd_seq_connect_from(midi.seq, midi.input_port, SND_SEQ_CLIENT_SYSTEM,
  ...
  s = pthread_create(&pid, &attr, &midi_loop, NULL);
