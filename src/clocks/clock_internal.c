...
static void *clock_internal_thread_run(void *p) {
  ...
  while (true) {
    ...
    if (clock_internal_restarted) {
      ...
      clock_update_source_reference(&clock_internal_reference, reference_beat,
                                    beat_duration);
...
void clock_internal_set_tempo(double bpm) {
  ...
  clock_internal_tempo.tick_duration =
      clock_internal_tempo.beat_duration / CLOCK_INTERNAL_TICKS_PER_BEAT;

  pthread_mutex_unlock(&clock_internal_tempo_lock);
}

void clock_internal_restart() { clock_internal_restarted = true; }

void clock_internal_stop() { clock_stop_from_source(CLOCK_SOURCE_INTERNAL); }
