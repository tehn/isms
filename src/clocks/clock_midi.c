...
static void clock_midi_handle_clock() {
  ...
  if (clock_midi_last_tick_time_set == false) {
    ...
    if (beat_duration > 4) { // assume clock stopped
      ...
      beat_duration_buf_pos =
          (beat_duration_buf_pos + 1) % DURATION_BUFFER_LENGTH;

      clock_midi_counter++;
      clock_midi_last_tick_time = current_time;

      double reference_beat = clock_midi_counter / 24.0;
      clock_update_source_reference(&clock_midi_reference, reference_beat,
                                    mean_sum);
...
static void clock_midi_handle_start() { clock_midi_counter = -1; }
